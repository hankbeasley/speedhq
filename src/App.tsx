...
import { Sliders, Volume2, VolumeX, Phone, Keyboard, Gamepad2, Sparkles, Navigation, Award, RotateCcw, Zap } from 'lucide-react';

export default function App() {
  const [gameState, setGameState] = useState<'lobby' | 'playing' | 'scores'>('lobby');

  // When fullscreen, the game fills the viewport: hide the header/footer chrome
  // and lock the page height so there is nothing to scroll.
  ...
  return (
    <main className={`bg-slate-950 text-slate-100 flex flex-col justify-between relative font-mono selection:bg-red-500 selection:text-white ${isFullscreen ? 'h-dvh overflow-hidden' : 'min-h-screen overflow-y-auto'}`}>
      ...
      {/* Corporate SpeedHQ branding Header (hidden in fullscreen to free space) */}
      <header className={`border-b border-slate-900 bg-slate-950/80 backdrop-blur-md px-6 py-4 flex items-center justify-between z-10 ${isFullscreen ? 'hidden' : ''}`}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3 font-mono">
                    <button
                      ...
                      className={`px-3 py-3 border text-xs rounded-xl flex flex-col items-center gap-1.5 cursor-pointer transition-all text-center ${
                    ...
                    <button
                      id="mode-dpad-btn"
                      onClick={() => handleUpdateSettings({ steeringMode: 'dpad' })}
                      className={`px-3 py-3 border text-xs rounded-xl flex flex-col items-center gap-1.5 cursor-pointer transition-all text-center ${
                ...
                <div className="space-y-3 font-mono pt-1">
                  <div>
                    <div className="flex justify-between items-center text-xs text-slate-400 mb-1.5 font-bold uppercase tracking-widest">
                      ...
                      <span className="text-emerald-400">{Math.round(settings.musicVolume * 100)}%</span>
                    </div>
                    <input
                      ...
                      onChange={(e) => handleUpdateSettings({ musicVolume: parseFloat(e.target.value) })}
      ...
      {gameState === 'playing' ? (
        <div className={`flex-1 min-h-0 flex flex-col justify-center items-center w-full h-full relative z-10 bg-slate-950 ${isFullscreen ? 'py-0' : 'py-4'}`}>
          <GameCanvas
            settings={settings}
      ...
      {/* Footer / Arcade branding credit stamp (hidden in fullscreen) */}
      <footer className={`border-t border-slate-900 bg-slate-950/60 p-4 text-center text-[10px] text-slate-600 select-none font-mono tracking-widest uppercase flex-col sm:flex-row items-center justify-between px-8 gap-4 ${isFullscreen ? 'hidden' : 'flex'}`}>
        <div>
