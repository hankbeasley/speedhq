...
// Curves should pressure the car only once speed builds. The previous tuning
// applied most of the curve force from the start of Coconut Coast, which made
// keyboard play feel like a constant left drift before the player had enough
// speed or steering authority to counter it.
const CURVE_PULL_START_SPEED = 90.0;
const CURVE_PULL_FULL_SPEED = 285.0;
...
export default function GameCanvas({ settings, updateSettings, onGameOver }: GameCanvasProps) {
  ...
  const updatePhysics = (dt: number, totalLength: number) => {
    ...
    if (!s.isCrashActive) {
      ...
      const curveForce01 = clamp((s.speed - CURVE_PULL_START_SPEED) / (CURVE_PULL_FULL_SPEED - CURVE_PULL_START_SPEED), 0, 1);
      const curvePull = currentSegment.curve * (0.05 + speed01 * 0.26) * curveForce01;
      const counterSteerAssist = clamp(-curvePull * 0.10, -0.22, 0.22);
